class FlashMarketService {
  ...
  generateMarkets(match) {
      ...
      if (!match || typeof match.fixture?.status?.elapsed !== 'number') {
          return { "Apostas Rápidas": [], "Aguardando...": [] };
      }

      const elapsed = Math.floor(match.fixture.status.elapsed);
      const fId = match.fixture.id;
      const statusShort = match.fixture.status.short;
      const period = match.fixture.status.period || (elapsed > 45 ? '2H' : '1H');

      // 1. Safety Lock: Interval / Paused / Finished
      if (['PAUSED', 'HT'].includes(statusShort)) {
          return { "Intervalo": [this.createIntervalMarket(fId)] };
      }
      if (['FINISHED', 'FT', 'AWARDED'].includes(statusShort)) {
          return { "Fim de Jogo": [] };
      }

      // 2. Stoppage Time Markets
      const isFirstHalfStoppage = (elapsed >= 45 && period === '1H');
      const isSecondHalfStoppage = (elapsed >= 90); // 2H Stoppage or FT pending

      if (isFirstHalfStoppage || isSecondHalfStoppage) {
          const suffix = isFirstHalfStoppage ? '1h' : '2h';
          return {
              "Acréscimos (Fim do Tempo)": [
                  this.createStoppageGoalMarket(fId, suffix, elapsed),
                  this.createStoppageCardMarket(fId, suffix, elapsed)
              ]
          };
      }

      // 3. Standard Markets (Corrected Math for Min 0+)
      // 1 Minute Flash (Next Full Minute)
      // If elapsed = 0 (00:00 - 00:59), next full minute is 1 (01:00 - 01:59).
      // Current minute is 0. Window starts at next integer.
      const nextMinStart = elapsed + 1;
      const flashMarket = this.createFlashGoalMarket(fId, nextMinStart);

      // 5 Minutes (Next Block)
      // If elapsed = 2. Block is 0. Next block starts at 5.
      // If elapsed = 5. Block is 1. Next block starts at 10.
      const currentBlock5 = Math.floor(elapsed / 5);
      const next5Start = (currentBlock5 + 1) * 5;
      const winner5Market = this.create5MinWinnerMarket(fId, next5Start);
      const goal5Market = this.create5MinGoalMarket(fId, next5Start);

      // 10 Minutes (Next Block)
      const currentBlock10 = Math.floor(elapsed / 10);
      const next10Start = (currentBlock10 + 1) * 10;
      const ou10Market = this.create10MinOverUnderMarket(fId, next10Start);

      return {
          "Apostas de 1 Minuto": [flashMarket].filter(Boolean),
          "Apostas de 5 Minutos": [winner5Market, goal5Market].filter(Boolean),
          "Apostas de 10 Minutos": [ou10Market].filter(Boolean)
      };
  }
